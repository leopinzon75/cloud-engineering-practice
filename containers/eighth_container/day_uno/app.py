...
# Busca esta línea en tu app.py y cámbiala por esta:
...
def s3_telemetry_streamer(s3_client, bucket, key, simulation_mode=False):
    if simulation_mode:
        ...
        # Aquí consume directo el S3 real de tu LocalStack contenedor
        try:
            ...
            if e.response['Error']['Code'] == 'NoSuchKey':
                ...
                # Auto-generación de las 1000 filas en el S3 de LocalStack si no existe
...
# =========================================================
# =========================================================
# BUCLE DE ORQUESTACIÓN ADAPTADO PARA KUBERNETES CRONJOB
# =========================================================
if __name__ == '__main__':
    # Inicialización de clientes Boto3 apuntando a LocalStack
    ...
    # 1. Generamos un ID único basado en el bloque del minuto actual para el CronJob
    execution_id = f"cron_batch_{datetime.now().strftime('%Y%m%d_%H%M')}"
    
    print(f"⏱️ Iniciando ejecución única de Kubernetes CronJob para el bloque: {execution_id}")

    # 2. Intentar capturar el candado distribuido en DynamoDB
    ...
    print("👋 Proceso terminado con éxito. El Pod de Kubernetes ya puede cerrarse limpiamente.")